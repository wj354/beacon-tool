...
{block name="title"}列表管理{/block}
{block name="caption"}工具-列表管理{/block}

{block name='list-header'}
    <div class="yee-list-header">
        <div class="yee-caption"><i class="icofont-listine-dots"></i> 工具-列表管理</div>
        <div class="yee-toolbar">
            ...
            <a id="add-btn" href="{url act='add' appId=$appId}" class="yee-btn red"><i class="icofont-patient-file"></i>添加列表</a>
        </div>
    </div>
{/block}
...
{block name='list-search'}
    <div class="yee-list-search">
        <form id="searchForm" yee-module="search-form" data-bind="#list">
            <div class="yee-cell">
                <label class="yee-section-label"><em></em>列表名称：</label>
            ...
            {literal left='<{' right='}>'}
                <div class="yee-cell fr">
                    <a href="<{url act='import' appId=$appId}>"
                       ...
                       data-maxmin="false"
                       data-auto-size="true"
                       class="yee-btn" on-success="window.location.href=Yee.url('<{url act='addImport' appId=$appId}>',{import:ret});"><i class="icofont-copy"></i>导入列表</a>
                </div>
            {/literal}
        </form>
    </div>
...
{block name='list-tab'}
    <div class="yee-tab">
        <ul yee-module="list-tab">
            <li{if $appId==0} class="curr"{/if}><a href="{url ctl='Lists' act='index' appId=0}" data-tab-index='0'>全部</a></li>
            {foreach from=$applist item="rs"}
                <li{if $appId==$rs.id} class="curr"{/if}><a href="{url ctl='Lists' act='index' appId=$rs.id}" data-tab-index='{$rs.id}'>{$rs.name}</a></li>
            {/foreach}
        </ul>
    </div>
{/block}
...
{block name='list-table'}
    <table id="list" class="yee-datatable" yee-module="datatable"
        ...
        <tr>
            ...
            <th align="left">列表名称</th>
            <th width="200" align="left">所属表单模型</th>
            <th width="200" align="left">数据表名称</th>
            <th width="450" align="right">操作</th>
        </tr>
        </thead>
        <tbody yee-template>
        <tr yee-each="list" yee-item="rs">
            ...
            <td :html="rs.key"></td>
            <td :html="rs.title"></td>
            <td :html="rs.formKey"></td>
            <td :html="rs.tbName"></td>
