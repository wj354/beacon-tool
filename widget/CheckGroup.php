...
class CheckGroup extends Form
{
    ...
    protected function load()
    {
        return [
            'bitComp' => [
                ...
                'default' => 0,
                'after-text' => '勾选按位存储',
                'box-onclick' => "if($(this).is(':checked')){\$('\#row_extend').find(':input[name*=type]:first').trigger('click');}",
                'dynamic' => [
                    [
                        'eq' => 1,
                        'hide' => 'type,names',
            ...
            'type' => [
                ...
                'dynamic' => [
                    ...
                    [
                        'eq' => 3,
                        'show' => 'method,param,sql,field,options',
                        'hide' => 'config,func',
                    ],
                    [
                        'eq' => 4,
    ...
    public static function export(array &$field, array $extend)
    {
        ...
        if(isset($extend['names'])){
            $names= Helper::convertArray($extend['names'], []);
            if(count($names)>0){
                $field['names']=$names;
            }
        }
