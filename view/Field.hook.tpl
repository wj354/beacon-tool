...
{hook fn='name' rs=null}<input value="{$rs.name}" type="text" style="width: 1px; opacity: 0; border: 0px;"/>
    <a href="javascript:;" class="v-copy">
        ...
        <span style="font-size: 14px;{if !$rs.dbfield}color: #999;{/if}{if $rs.close}color: #ccc;{/if}{if $rs.offEdit}text-decoration:underline;{/if}{if $rs.viewClose||$rs.viewHide}font-style:italic;{/if}">{$rs.name}</span>
    </a>
{/hook}
...
{hook fn='_viewClose' rs=null}{if $rs.viewClose||$rs.viewHide}Hide{/if}{/hook}
...
{hook fn='_operate' rs=null}
    <a href="{url act='edit' formId=$this->formId id=$rs.id  appId=$appId}" class="yee-btn blue-bd"><i class="icofont-edit"></i>编辑</a>
